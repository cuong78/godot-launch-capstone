...
colors:
  ...
  stone: "#434345"
  on-dark: "#f4f7fb"
  on-dark-mute: "rgba(244,247,251,0.72)"
  canvas: "#06090f"
  surface: "#090e16"
  surface-elevated: "#0e1520"
  surface-card: "#141e2b"
  button-fg: "#1a2635"
  hairline: "rgba(96, 119, 148, 0.34)"
  hairline-soft: "rgba(96, 119, 148, 0.15)"
  hairline-strong: "rgba(125, 211, 252, 0.28)"
  accent-blue: "#38bdf8"
  ...
  hero-stripe-start: "#fbbf24"
  hero-stripe-end: "#38bdf8"
  key-bg-start: "#141e2b"
  key-bg-end: "#0e1520"
...
Raycast's marketing site reads like an extended product screenshot. The chrome IS the in-product command palette at marketing scale: pure near-black canvas (`{colors.canvas}` — `#06090f`), hairline 1px borders (`{colors.hairline}` — `rgba(96, 119, 148, 0.34)`), command-palette-style cards with rounded corners between 6 and 16px, Inter typography with the **ss03 stylistic set enabled site-wide** (a single character — the alternate `g` — that gives Raycast's typography its signature subtle distinction), a single amber CTA pill that anchors every primary action, and small splashes of saturated accent reserved for category illustrations.

The system has effectively one surface mode — dark — with a faint three-step surface ladder (`{colors.canvas}` → `{colors.surface}` → `{colors.surface-elevated}` → `{colors.surface-card}`) carrying cards, in-card panels, and key-cap glyph backgrounds. The signature decorative moment is an **amber-to-cyan diagonal-stripe gradient band** across the very top of the home page hero, used as a launch-banner motif behind the headline. Beyond that single moment, color in the chrome is reserved for category accents inside extension and feature illustrations: Hacker News yellow, Slack red, Linear green, info blue.
...
**Key Characteristics:**
- Single dark surface mode with a 4-step surface ladder: `{colors.canvas}` (#06090f) → `{colors.surface}` (#090e16) → `{colors.surface-elevated}` (#0e1520) → `{colors.surface-card}` (#141e2b)
- Amber CTA pill (`{colors.primary}` — #fbbf24) is the universal primary action; everything else is monochrome dark
...
- Hairline 1px borders (`{colors.hairline}` — rgba(96, 119, 148, 0.34)) carry every card edge; there are no drop shadows in the system
...
- Signature diagonal-stripe gradient band at the very top of the hero — three angled stripes in `{colors.hero-stripe-start}` → `{colors.hero-stripe-end}`, used once per page maximum
...
### Brand & Accent
- **Amber** (`{colors.primary}` — `#fbbf24`): the universal primary CTA pill background. "Download" / "Install Extension" / "Get Pro" — every primary action carries it.
- **Amber Pressed** (`{colors.primary-pressed}` — `#d97706`): pressed-state for the primary pill — a single notch dimmer.
- **On Primary** (`{colors.on-primary}` — `#0f172a`): deep slate text on the amber CTA.

### Surface
- **Canvas** (`{colors.canvas}` — `#06090f`): pure-near-black page background. The dominant surface across every page.
- **Surface** (`{colors.surface}` — `#090e16`): card and elevated panel background — one notch lighter than canvas.
- **Surface Elevated** (`{colors.surface-elevated}` — `#0e1520`): button-tertiary fill, text-input fill, store-search-bar fill, pill-tab-active fill.
- **Surface Card** (`{colors.surface-card}` — `#141e2b`): app-icon-tile background, keycap fill, command-palette row hover.
- **Button FG (in-card)** (`{colors.button-fg}` — `#1a2635`): rare deep-card variant used inside featured pricing tier card backgrounds.
- **Hairline** (`{colors.hairline}` — `rgba(96, 119, 148, 0.34)`): the universal 1px card border. Carries every card edge across every page.
- **Hairline Soft** (`{colors.hairline-soft}` — `rgba(96, 119, 148, 0.15)`): even fainter border on translucent over-image overlays.
- **Hairline Strong** (`{colors.hairline-strong}` — `rgba(125, 211, 252, 0.28)`): stronger 1px divider where a regular hairline reads as too soft.

### Text
- **Ink** (`{colors.ink}` — `#f4f7fb`): primary headlines on dark canvas. Slightly off-white for tonal coherence with the near-black background.
- **Body** (`{colors.body}` — `#b1bdcc`): default paragraph text and inline-link color.
- **Charcoal** (`{colors.charcoal}` — `#b1bdcc`): subtly brighter body where ink reads too soft.
- **Mute** (`{colors.mute}` — `#718096`): metadata, footer link text, secondary captions.
- **Ash** (`{colors.ash}` — `#718096`): disabled-state text, lowest-emphasis utility.
- **Stone** (`{colors.stone}` — `#434345`): least-emphasis caption text and disabled icon color.
- **On Dark** (`{colors.on-dark}` — `#f4f7fb`): interactive-state primary text (button label, focused tab).
- **On Dark Mute** (`{colors.on-dark-mute}` — `rgba(244,247,251,0.72)`): translucent secondary text on dark surfaces.

### Semantic
- **Accent Blue** (`{colors.accent-blue}` — `#38bdf8`) + **Soft** (`{colors.accent-blue-soft}` — `rgba(56,189,248,0.15)`): info and informational badge — used inside feature illustrations and the rare "New" pill.
- **Accent Red** (`{colors.accent-red}` — `#ef4444`) + **Soft** (`{colors.accent-red-soft}` — `rgba(239,68,68,0.15)`): destructive/error indicator + Slack/Apple category accent in extension illustrations.
- **Accent Green** (`{colors.accent-green}` — `#10b981`) + **Soft** (`{colors.accent-green-soft}` — `rgba(16,185,129,0.15)`): success state + productivity category accent in extension illustrations.
- **Accent Yellow** (`{colors.accent-yellow}` — `#fbbf24`) + **Soft** (`{colors.accent-yellow-soft}` — `rgba(251,191,36,0.15)`): "warning" semantic + the Hacker News orange-yellow that appears as the most prominent accent illustration on the home page hero.

### Brand Gradient
- **Hero Stripe Gradient** — three diagonal stripes layered across the very top of the home page hero, fading from `{colors.hero-stripe-start}` (`#fbbf24`) to `{colors.hero-stripe-end}` (`#38bdf8`). The system's only chromatic gradient on chrome — used once per page maximum and reserved for hero launch-banner moments.
- **Keycap Gradient** — the small key-glyph background uses a subtle linear-gradient from `{colors.key-bg-start}` (`#141e2b`) to `{colors.key-bg-end}` (`#0e1520`) that gives Raycast's keycap UI its slight 3D-key feel.
...
- Use `{colors.primary}` (amber pill) for every primary CTA. There is no second primary color — amber IS the brand action.
...
- Reserve `{colors.hero-stripe-start}` → `{colors.hero-stripe-end}` amber-to-cyan gradient for the hero band exactly once per page. Never repeat the stripe gradient deeper in the page.
...
- Don't replace `{colors.primary}` (amber) with an unaligned accent for the primary CTA. Pure amber is the brand action color.
