...
export const AdminFileManagementPanel: React.FC = () => {
  ...
  const handleDownload = async (file: UploadedFileResponse, inline: boolean = false) => {
    ...
    try {
      ...
      let message = err.message || 'Lỗi kết nối';
      ...
      alert('Không thể tải tập tin: ' + message);
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="p-4 bg-slate-50/50 dark:bg-slate-900/30 border border-slate-200/60 dark:border-slate-800/60 rounded-2xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Được định tuyến theo loại file</p>
          </div>
        </div>
        ...
        <div className="p-4 bg-slate-50/50 dark:bg-slate-900/30 border border-slate-200/60 dark:border-slate-800/60 rounded-2xl flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-600 dark:text-slate-400 mt-1">Lưu trữ local & tối ưu hóa dung lượng</p>
          </div>
        </div>
      </div>

      {/* Category Selection Tab-pills */}
      <div className="flex flex-wrap bg-slate-100/50 dark:bg-slate-950/40 p-1.5 rounded-2xl border border-slate-200/50 dark:border-slate-800/40 gap-1.5">
        {categories.map((cat) => (
          <button
            key={cat.value}
            onClick={() => {
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-semibold transition-all duration-200 cursor-pointer ${
              category === cat.value
                ...
                : 'text-slate-500 hover:text-slate-700 dark:hover:text-slate-300 hover:bg-slate-200/40 dark:hover:bg-slate-900/45'
            }`}
          >
            {cat.icon}
            {cat.label}
      ...
      <form onSubmit={handleSearchSubmit} className="flex gap-2 max-w-md">
        ...
        <button
          type="submit"
          className="px-4 py-2 bg-amber-400 hover:bg-amber-500 text-slate-950 font-bold rounded-xl text-xs shadow-md shadow-amber-400/10 hover:shadow-amber-400/20 transition-all active:scale-95 cursor-pointer"
        >
          Tìm kiếm
        </button>
      </form>
                ...
                {files.map((file) => (
                  <tr key={file.id} className="hover:bg-slate-50/40 dark:hover:bg-slate-900/5 transition-colors">
                    ...
                    <td className="p-3 max-w-xs">
                      <div className="font-semibold truncate text-slate-850 dark:text-slate-150" title={file.fileName}>
                        {file.fileName}
                      </div>
