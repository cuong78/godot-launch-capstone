  ...
  const load = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.message || t('externalPublish.loadStatusError'));
      }
    } catch (e: any) {
      setError(e?.response?.data?.message || e?.message || t('externalPublish.loadStatusCatchError'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        setError(res.message || t('externalPublish.uploadError'));
      }
    } catch (e: any) {
      setError(e?.response?.data?.message || e?.message || t('externalPublish.uploadCatchError'));
    } finally {
      setUploading(false);
    }
  };
        ...
        <div className="p-3 rounded-lg bg-white/60 dark:bg-slate-950/30 border border-slate-200/60 dark:border-slate-800/60 space-y-1.5">
          <div className="flex items-center gap-1.5 text-xs font-bold">
            ...
            <span className="uppercase">
              {publish.status === 'live' ? t('externalPublish.statusLive')
                : publish.status === 'submitted' ? t('externalPublish.statusSubmitted')
                : publish.status === 'rejected' ? t('externalPublish.statusRejected')
                : publish.status}
            </span>
          </div>
          {publish.versionNumber && (
            <p className="text-[11px] text-slate-500 dark:text-slate-400">{t('externalPublish.versionLabel', { value: publish.versionNumber })}</p>
          )}
          {publish.storeUrl && (
            <a
            ...
            >
              <ExternalLink size={12} /> {t('externalPublish.viewStore')}
            </a>
          )}
      ...
      {gameStatus === 'awaiting_store_build' && (publish?.status !== 'submitted') && (
        <div className="space-y-2 pt-1">
          ...
          <div>
            <input
              type="text"
              placeholder={t('externalPublish.shortDescriptionPlaceholder')}
          ...
          <textarea
            placeholder={t('externalPublish.changelogPlaceholder')}
            value={changelog}
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <Upload size={13} /> {t('externalPublish.chooseBuild')}
            </label>
            <span className="text-xs text-slate-500 font-mono truncate max-w-[160px]">
              {file ? file.name : t('externalPublish.noBuildSelected')}
            </span>
          </div>
          <div className="flex items-center gap-2">
            <input
            ...
            <label
              ...
              className="px-3 py-2 bg-slate-100 dark:bg-slate-900 hover:bg-slate-200 border border-slate-300 dark:border-slate-800 rounded-lg text-xs font-semibold cursor-pointer flex items-center gap-1.5"
            >
              <Upload size={13} /> {t('externalPublish.chooseFeatureGraphic')}
            </label>
            <span className="text-xs text-slate-500 font-mono truncate max-w-[160px]">
              {featureGraphic ? featureGraphic.name : t('externalPublish.noFeatureGraphicSelected')}
            </span>
          </div>
          <button
          ...
          >
            ...
            {uploading ? t('externalPublish.uploading') : t('externalPublish.uploadSubmit')}
          </button>
        </div>
      )}
    </div>
