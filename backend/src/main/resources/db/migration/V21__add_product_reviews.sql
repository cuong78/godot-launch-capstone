...
CREATE UNIQUE INDEX uq_user_game_review ON public.reviews(user_id, game_id) WHERE game_id IS NOT NULL;
CREATE UNIQUE INDEX uq_user_asset_review ON public.reviews(user_id, asset_id) WHERE asset_id IS NOT NULL;

-- Indexes for performance when querying reviews
CREATE INDEX idx_reviews_game_id ON public.reviews USING btree (game_id) WHERE game_id IS NOT NULL;
CREATE INDEX idx_reviews_asset_id ON public.reviews USING btree (asset_id) WHERE asset_id IS NOT NULL;
CREATE INDEX idx_reviews_user_id ON public.reviews USING btree (user_id);

-- Add rating cached statistics to games and assets tables
ALTER TABLE public.games ADD COLUMN average_rating numeric(3,2) DEFAULT 0.00 NOT NULL;
ALTER TABLE public.games ADD COLUMN review_count integer DEFAULT 0 NOT NULL;

ALTER TABLE public.assets ADD COLUMN average_rating numeric(3,2) DEFAULT 0.00 NOT NULL;
ALTER TABLE public.assets ADD COLUMN review_count integer DEFAULT 0 NOT NULL;
